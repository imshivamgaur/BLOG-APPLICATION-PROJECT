...
const updateProfile = async (req, res) => {
  try {
    ...
    if (oldpassword && newpassword) {
     ...
     if (req.file) {
      // Remove the old image if necessary (if stored locally or on Cloudinary)
      if (ExistUser.profile) {
        // Example for Cloudinary: await cloudinary.uploader.destroy(ExistUser.profile);
        // Example for local storage:
        const oldImagePath = `uploads/${ExistUser.profile}`;
      ...
      const newImagePath = req.file.filename; // Or handle Cloudinary upload here
      ExistUser.profile = newImagePath;
    }
